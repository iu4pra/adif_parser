...
def parse_adif_string(_adif: str):
    ...
    while len(_adif) > 0 and cursor < (len(_adif) - 1):
        ...
        if match:
            # Match found
            logging.debug(f"Match found at position {match.start()} to {match.end()}")
            ...
            if field["len"] > 0:
                field["value"] = _adif[match.end() : match.end() + field["len"]]
            else:
                field["value"] = None
            ...
            logging.debug(f"No match found starting from {cursor} to {len(_adif)-1}")
